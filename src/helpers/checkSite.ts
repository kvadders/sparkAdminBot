...
const STATUS_MAP = [
  { check: (s: number) => s >= 200 && s < 300, emoji: '✅', text: 'Available' },
  { check: (s: number) => s >= 300 && s < 400, emoji: '🔄', text: 'Redirect' },
  { check: (s: number) => s >= 400 && s < 500, emoji: '⚠️', text: 'Client Error' },
  { check: (s: number) => s >= 500, emoji: '🚨', text: 'Server Error' },
];
...
export async function checkAllSitesCommand(ctx: MyContext) {
